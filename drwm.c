...
struct river_xkb_config_v1 *xkb_config_v1;

struct river_xkb_keymap_v1 *global_keymap;
...
// Config event handler and keyboard configuration

static struct river_xkb_keymap_v1 *
create_keymap(struct xkb_rule_names rules)
{
	char *keymap_str = NULL;
	int keymap_fd = -1;
	void *data = NULL;
	struct xkb_keymap *keymap = xkb_keymap_new_from_names2(
			xkb_context, &rules, XKB_KEYMAP_FORMAT_TEXT_V2,
			XKB_KEYMAP_COMPILE_NO_FLAGS);
	if (keymap == NULL) {
		goto defer;
	}

	keymap_str = xkb_keymap_get_as_string2(keymap,
			XKB_KEYMAP_FORMAT_TEXT_V2,
			XKB_KEYMAP_SERIALIZE_NO_FLAGS);
	xkb_keymap_unref(keymap);
	int keymap_str_len = strlen(keymap_str) + 1;
	keymap_fd = memfd_create("drwm-keymap",
			MFD_CLOEXEC | MFD_ALLOW_SEALING);
	if (keymap_fd == -1) goto defer;
	if (ftruncate(keymap_fd, keymap_str_len) < 0) goto defer;

	data = mmap(NULL, keymap_str_len, PROT_READ | PROT_WRITE,
		MAP_SHARED, keymap_fd, 0);
	if (data == MAP_FAILED) goto defer;

	memcpy(data, keymap_str, keymap_str_len);
	free(keymap_str);
	keymap_str = NULL;
	if (munmap(data, keymap_str_len) < 0) goto defer;
	if (fcntl(keymap_fd, F_ADD_SEALS, F_SEAL_SHRINK | F_SEAL_GROW |
			F_SEAL_WRITE | F_SEAL_SEAL) < 0) goto defer;

	return river_xkb_config_v1_create_keymap(xkb_config_v1,
		keymap_fd, XKB_KEYMAP_FORMAT_TEXT_V2);

defer:
	if (keymap_fd != -1) close(keymap_fd);
	if (keymap_str != NULL) free(keymap_str);
	return NULL;
}

static void xkb_config_handle_finished(void *data,
		struct river_xkb_config_v1 *config) {}

static void xkb_config_handle_xkb_keyboard(void *data,
		struct river_xkb_config_v1 *config,
		struct river_xkb_keyboard_v1 *keyboard)
{
	if (global_keymap) {
		river_xkb_keyboard_v1_set_keymap(keyboard, global_keymap);
	}
}

struct river_xkb_config_v1_listener xkb_config_listener = {
	.finished = xkb_config_handle_finished,
	.xkb_keyboard = xkb_config_handle_xkb_keyboard,
};
...
static void handle_global(void *data, struct wl_registry *registry, uint32_t name, const char *interface, uint32_t version) {
	if (strcmp(interface, river_window_manager_v1_interface.name) == 0 && version >= 4) {
		...
		xkb_config_v1 = wl_registry_bind(registry, name, &river_xkb_config_v1_interface, 1);
	}
}
...
int main(void) {
	...
	if (window_manager_v1 == NULL || xkb_bindings_v1 == NULL || layer_shell_v1 == NULL || xkb_config_v1 == NULL) {
	...
	global_keymap = create_keymap(kb_rules);

	if (xkb_config_v1) river_xkb_config_v1_add_listener(xkb_config_v1, &xkb_config_listener, NULL);
